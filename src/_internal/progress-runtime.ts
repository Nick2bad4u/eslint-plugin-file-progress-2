...
import type {
    FileProgressRuleContext,
    FileProgressRuleListener,
    FileProgressRuleMetaData,
...
interface CreateProgressRuleOptions {
    description: string;
    liveMode: "file" | "generic" | "summary-only";
    recommended?: boolean;
    ruleId: "activate" | "compact" | "summary-only";
    url: string;
}
...
const getProgressSetting = (
    settings: FileProgressRuleContext["settings"]
): unknown => {
...
const applyRuleMode = (
    settings: NormalizedProgressSettings,
    liveMode: CreateProgressRuleOptions["liveMode"]
...
export const createProgressRule = (
    ...
    create(context): FileProgressRuleListener {
        if (lintStartedAt === 0) {
            lintStartedAt = Date.now();
        }
        lintedFileCount += 1;

        const settings = applyRuleMode(
            normalizeSettings(getProgressSetting(context.settings)),
            options.liveMode
        );
        ...
        if (options.liveMode === "summary-only") {
            return {};
        }

        if (options.liveMode === "generic") {
