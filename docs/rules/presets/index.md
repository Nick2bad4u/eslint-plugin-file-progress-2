...
- Choose **recommended** for standard local CLI progress.
- Choose **recommended-ci** when CI should stay quiet.
- Choose **recommended-detailed** when you want richer end-of-run stats.
- Choose **recommended-compact** when file names are unnecessary noise.
- Choose **recommended-summary-only** when only the final summary matters.
- Choose **recommended-tty** when the same config is reused in non-interactive environments.
- Choose **recommended-ci-detailed** when CI should avoid live output but still print a final detailed summary.
...
| [`recommended-compact`](./recommended-compact.md) | [`file-progress/compact`](../compact.md) | defaults | Use compact live mode without per-file paths. |
| [`recommended-summary-only`](./recommended-summary-only.md) | [`file-progress/summary-only`](../summary-only.md) | defaults | Print only the final summary line. |
