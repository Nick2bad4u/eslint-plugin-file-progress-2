...
const collectAddedMenuLinks = (record: MutationRecord): HTMLAnchorElement[] => {
...
function applySidebarLabelTokenColoring(): CleanupFunction {
    ...
    const sidebarInteractionController = new AbortController();
        ...
        sidebarMenu === null
            ...
            : new MutationObserver((records) => {
                  ...
                  for (const record of records) {
                      addedLinks.push(...collectAddedMenuLinks(record));
                  }
    ...
    sidebarMenu?.addEventListener("click", handleSidebarInteraction, {
        signal: sidebarInteractionController.signal,
    });

    return (): void => {
        sidebarMenu?.removeEventListener("click", handleSidebarInteraction);
        sidebarInteractionController.abort();
...
function applyThemeToggleAnimation(): CleanupFunction {
    ...
    const themeToggleController = new AbortController();
    ...
    themeToggle.addEventListener("click", handleClick, {
        signal: themeToggleController.signal,
    });
...
function getRuleNumberPrefix(
    ...
    const match = /^(?<numberToken>\d{2,3}) (?<remainder>\S.*)$/v.exec(label);
...
function initializeAdvancedFeatures(): CleanupFunction {
    ...
    const isPrefersReducedMotion = browserWindow.matchMedia(
        "(prefers-reduced-motion: reduce)"
    ).matches;
    const cleanupFunctions: CleanupFunction[] = isPrefersReducedMotion
...
/**
 * Processes a single sidebar link element for rule-number prefixing.
 *
 * @param link - Sidebar anchor element.
 * @param linkLabel - The visible text label for this link.
 * @param mutations - Mutation tracking array to push into.
 */
function processNumberedRuleSidebarLink(
...
/**
 * Processes a single sidebar link element for runtime kind prefixing.
 *
 * @param link - Sidebar anchor element.
 * @param linkLabel - The visible text label for this link.
 * @param mutations - Mutation tracking array to push into.
 *
 * @returns `true` if the link was handled and the caller should skip other
 *   checks.
 */
function shouldProcessRuntimeSidebarLink(
    link: HTMLAnchorElement,
    linkLabel: string,
    mutations: SidebarLabelMutation[]
): boolean {
